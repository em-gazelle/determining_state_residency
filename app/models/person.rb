class Person < ActiveRecord::Base
	...
	has_many :trips, dependent: :destroy do
		...
		def residency_conclusion
			return "Oops! Looks like you haven't added any information on how you've spent the year... you've gotta give a little to get! Right now all we can say is the standard: you'll need to spend at least #{majority_of_year} days in #{@desired_state_of_residency} in order to achieve residency!" if self.blank?
		...
		def trip_summary_data(desired_state_of_residency, leap_year)
			@leap_year = leap_year
